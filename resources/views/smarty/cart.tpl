                ...
                <div class="lg:col-span-8">
                    <div class="bg-white rounded-lg shadow-sm p-6">
                        ...
                        <div class="flex items-center justify-between mb-6">
                            <h1 class="text-2xl font-medium text-gray-900">Cart ({$cart_count} products)</h1>
                        ...
                        <div class="divide-y divide-gray-200">
                            {if $cart|@count > 0}
                                {foreach from=$cart item=item key=product_id}
                                    <div class="py-6 cart-item" data-product-id="{$product_id}">
                                        <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                            <!-- Product Info -->
                                            <div class="md:col-span-6 flex items-center space-x-4">
                                                <div class="w-16 h-16 bg-gray-100 rounded-lg flex items-center justify-center">
                                                    {if isset($item.image) && $item.image}
                                                        <img src="{asset path=$item.image}" alt="{$item.name|default:'Product'}" class="w-15 h-15 object-cover rounded-lg">
                                                    {else}
                                                ...
                                                <div>
                                                    <h3 class="font-medium text-gray-900">{$item.name|default:'Product'}</h3>
                                            ...
                                            <div class="md:col-span-2 flex items-center justify-center">
                                                <div class="flex items-center border border-gray-300 rounded-lg">
                                                    <button class="p-2 hover:bg-gray-50 text-gray-400 decrease-quantity" data-product-id="{$product_id}">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                                d="M20 12H4" />
                                                        </svg>
                                                    </button>
                                                    <span class="px-4 py-2 text-sm font-medium item-quantity">{$item.quantity|default:1}</span>
                                                    <button class="p-2 hover:bg-gray-50 text-gray-400 increase-quantity" data-product-id="{$product_id}">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                                d="M12 4v16m8-8H4" />
                                                        </svg>
