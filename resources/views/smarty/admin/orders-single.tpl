...
{block name="content"}
    ...
    <div class="bg-gray-50 min-h-screen py-8 md:py-20">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-6">
            ...
            <div class="flex flex-col lg:flex-row gap-6 lg:gap-8" data-order-id="{$order.id}">
                {* Sidebar *}
                ...
                <div class="flex-1">
                    <div class="bg-white rounded-lg p-4 md:p-6 border border-gray-200">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 md:mb-8 gap-4">
                            <h1 class="text-2xl md:text-3xl font-bold text-gray-900">Order #{$order.number|default:'1234'}</h1>
                                ...
                                <div class="bg-white rounded-lg border border-gray-200 p-4 md:p-6">
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm md:text-base text-gray-900">{$order.date_of_sale|default:'09/30/2022'}</p>
                                        </div>
                                        <div>
                                            <label class="block text-xs md:text-sm font-medium text-gray-500 mb-1">Account
                                                number</label>
                                            <p class="text-sm md:text-base text-gray-900">{$order.account_number|default:'12345678912'}</p>
                                        </div>
                                        {* Second Row *}
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm md:text-base text-gray-900">{$order.city|default:'Toronto'}</p>
                                        </div>
                                        {* Third Row *}
                                        <div>
                                            <label class="block text-xs md:text-sm font-medium text-gray-500 mb-1">Province</label>
                                            <p class="text-sm md:text-base text-gray-900">{$order.province|default:'Ontario'}</p>
                                        </div>
                                        <div>
                                            <label class="block text-xs md:text-sm font-medium text-gray-500 mb-1">Postal code</label>
                                            <p class="text-sm md:text-base text-gray-900">{$order.postal_code|default:'123456'}</p>
                                        </div>
                                        {* Fourth Row *}
                                        <div>
                                            <label class="block text-xs md:text-sm font-medium text-gray-500 mb-1">Customer full
                                                name</label>
                                            <p class="text-sm md:text-base text-gray-900">{$order.customer_name|default:'John Smith'}</p>
                                        </div>
                                        <div>
                                            <label class="block text-xs md:text-sm font-medium text-gray-500 mb-1">Customer phone
                                                number</label>
                                            <p class="text-sm md:text-base text-gray-900">{$order.customer_phone|default:'+1 111 111 1234'}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center space-x-3 md:space-x-4">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <h4 class="font-medium text-gray-900 text-sm md:text-base truncate">{$item.name}</h4>
                                                <p class="text-xs md:text-sm text-gray-500">₦{$item.price|string_format:"%.2f"}</p>
                                            </div>
                                        </div>
                                        <div class="flex items-center justify-between sm:justify-end space-x-4 md:space-x-8">
                                            <span class="text-sm md:text-base text-gray-900">x{$item.quantity}</span>
                                            <span
                                                class="font-medium text-sm md:text-base text-gray-900 min-w-[60px] md:min-w-[80px] text-right">₦{$item.total|string_format:"%.2f"}</span>
                                        </div>
                                    </div>
                                {foreachelse}
