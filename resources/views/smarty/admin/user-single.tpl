            ...
            <nav class="flex mb-6 overflow-x-auto" aria-label="Breadcrumb">
                <ol class="flex items-center space-x-4 min-w-max">
                    ...
                    <li class="flex items-center">
                        ...
                        <span class="ml-4 text-black font-medium truncate">{$user_data.name}</span>
                    </li>
                </ol>
            </nav>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2 md:space-y-3">
                                        <div>
                                            ...
                                            <p class="text-xs md:text-sm text-gray-900 truncate">{$user_data.name}</p>
                                        </div>
                                        <div>
                                            <label class="text-xs md:text-sm font-medium text-gray-500">Email</label>
                                            <p class="text-xs md:text-sm text-gray-900 truncate">{$user_data.email}</p>
                                        </div>
                                        <div>
                                            <label class="text-xs md:text-sm font-medium text-gray-500">Joined</label>
                                            <p class="text-xs md:text-sm text-gray-900">{$user_data.created_at}</p>
                                        </div>
                                        <div>
                                            <label class="text-xs md:text-sm font-medium text-gray-500">Email Verification</label>
                                            <p class="text-xs md:text-sm text-gray-900">{$user_data.email_verified_at}</p>
                                        </div>
                                    </div>
                                </div>
                                <div>
                                    ...
                                    <div class="space-y-2 md:space-y-3">
                                        <div>
                                            ...
                                            <p class="text-xs md:text-sm font-semibold text-gray-900">{$user_data.orders_count}</p>
                                        </div>
                                        <div>
                                            <label class="text-xs md:text-sm font-medium text-gray-500">Total Spent</label>
                                            <p class="text-xs md:text-sm font-semibold text-gray-900">₦{$user_data.total_spent|number_format:2}</p>
                                        </div>
                                        <div>
                                            <label class="text-xs md:text-sm font-medium text-gray-500">Billing Addresses</label>
                                            <p class="text-xs md:text-sm font-semibold text-gray-900">{$user_data.billing_addresses_count}</p>
                                        </div>
                                        <div>
                                            <label class="text-xs md:text-sm font-medium text-gray-500">Payment Methods</label>
                                            <p class="text-xs md:text-sm font-semibold text-gray-900">{$user_data.payment_methods_count}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="p-4 md:p-6">
                                <div class="flex flex-wrap gap-3 md:gap-4">
                                    {if $user_data.twitter_handle}
                                        <a href="https://twitter.com/{$user_data.twitter_handle}" target="_blank" class="flex items-center text-blue-500 hover:text-blue-700 text-sm md:text-base">
                                            ...
                                            <span class="truncate">@{$user_data.twitter_handle}</span>
                                        </a>
                                    {/if}
                                    {if $user_data.facebook_handle}
                                        <a href="https://facebook.com/{$user_data.facebook_handle}" target="_blank" class="flex items-center text-blue-600 hover:text-blue-800 text-sm md:text-base">
                                            <svg class="w-4 h-4 md:w-5 md:h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                                            </svg>
                                            <span class="truncate">{$user_data.facebook_handle}</span>
                                        </a>
                                    {/if}
                                    {if $user_data.linkedin_handle}
                                        <a href="https://linkedin.com/in/{$user_data.linkedin_handle}" target="_blank" class="flex items-center text-blue-700 hover:text-blue-900 text-sm md:text-base">
                                            <svg class="w-4 h-4 md:w-5 md:h-5 mr-2" fill="currentColor" viewBox="0 0 24 24">
                                                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                                            </svg>
                                            <span class="truncate">{$user_data.linkedin_handle}</span>
                                        </a>
                                    {/if}
                                </div>
                            </div>
                                        ...
                                        {foreach $user_data.orders as $order}
                                            <tr class="hover:bg-gray-50">
                                                <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                                    <a href="{$base_url}/admin/orders/{$order.id}" class="text-xs md:text-sm font-medium text-gray-900 hover:text-blue-800">
                                                        #{$order.order_number}
                                                    </a>
                                                </td>
                                                <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                                    <span class="text-xs md:text-sm text-gray-600">{$order.created_at|date_format:"%b %d, %Y"}</span>
                                                </td>
                                                <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                                    <span class="text-xs md:text-sm font-semibold text-gray-900">₦{$order.total|number_format:2}</span>
                                                </td>
                                                <td class="px-3 md:px-6 py-4 whitespace-nowrap">
                                                    ...
                                                    {else}
                                                        <span class="inline-flex items-center px-2 py-0.5 md:px-2.5 md:py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">{$order.status|capitalize}</span>
                                                    {/if}
                                                </td>
                                            </tr>
                                        {/foreach}
                            ...
                            <div class="p-4 md:p-6">
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
                                    {foreach $user_data.billing_addresses as $address}
                                        <div class="border border-gray-200 rounded-lg p-3 md:p-4 {if $address.is_default}bg-blue-50 border-blue-200{/if}">
                                            ...
                                            {/if}
                                            <div class="space-y-1">
                                                <p class="text-xs md:text-sm text-gray-900">{$address.address_line_1}</p>
                                                {if $address.address_line_2}
                                                    <p class="text-xs md:text-sm text-gray-900">{$address.address_line_2}</p>
                                                {/if}
                                                <p class="text-xs md:text-sm text-gray-900">{$address.city}, {$address.state} {$address.postal_code}</p>
                                                <p class="text-xs md:text-sm text-gray-900">{$address.country}</p>
                                            </div>
                                        </div>
                                    {/foreach}
                                </div>
